...
% final_state(Fin), coordonnees([1,1], Fin, X).
%final_state(Fin), nth1(L,Fin,Ligne), nth1(C,Ligne,a)
   %*************
   % HEURISTIQUES
   %*************
...
%   heuristique2(U, H).  % ensuite utilisez plutot l'heuristique 2  
   ...
   % même piece.
    ...
    heuristique1(U, H) :-
        ...
        T is L*L,
        final_state(Fin),
        findall(P, bienPlace(P, U, Fin), X),
        length(X, L1),
        H is T - L1 - 1.
%   initial_state(Ini), final_state(Fin), heuristique1(Ini, H).
%   initial_state(Ini), final_state(Fin), heuristique1(Ini, H).
...
%   H = 4.
   ...
   % entre sa position courante et sa positon dans l'etat final
    dm(P, U, F, D) :-
        coordonnees([Li,Ci], U, P),
        coordonnees([Lf,Cf], F, P),
        abs(Lf - Li, X),
        abs(Cf - Ci, Y),
        D is X + Y.
% initial_state(Ini), final_state(Fin), dm(h, Ini, Fin, D).

    dists(U,D) :-
        final_state(Fin),
        coordonnees(_, Fin, El), % find all elements on the array
        El \= vide, % exclude vide
        dm(El, U, Fin, D).

    heuristique2(U, H) :-
        findall(D, dists(U, D), Dists),
        sum_list(Dists, H).

%initial_state(Ini), final_state(Fin), heuristique2(Ini, H).
%Ini = [[b, h, c], [a, f, d], [g, vide, e]],
%Fin = [[a, b, c], [h, vide, d], [g, f, e]],
%H = 5.
...
% final_state(Fin), coordonnees(_, Fin, El), El \= vide.
% initial_state(Ini), final_state(Fin), heuristique2(Ini, H).

bienPlace(vide, _, _) :- false.
bienPlace(P, Ini, Fin) :-
    nth1(L,Ini,Ligne),
    nth1(C,Ligne, P),
    nth1(L,Fin,Ligne2),
...
%initial_state(Ini), findall(P,  rule(P, 1, Ini, _), X).

% Ini: initial state
% A: step to do to get to S
% S: child state
follows(Ini, [A,S], Poids) :-
    rule(A, Poids, Ini, S).

%initial_state(Ini), follows(Ini, [up, S]).
%initial_state(Ini), follows(Ini, [A, [[b, h, c], [a, vide, d], [g, f, e]]]).
%initial_state(Ini), follows(Ini, C).

all_follows(Ini, S) :-
    findall(C, follows(Ini, C, _), S).

%initial_state(Ini), all_follows(Ini, X).

%initial_state(Ini), final_state(Fin), findall(P, bienPlace(P, Ini, Fin), X).
